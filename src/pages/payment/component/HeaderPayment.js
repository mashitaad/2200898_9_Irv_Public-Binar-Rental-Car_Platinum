...
function HeaderPayment({ navigateBack, set, bankName, padingBottom }) {
  ...
  const renderIcons = () => {
    ...
    return (
      <>
        {icon1} Pilih Metode <img src={rectangle} alt="rectangle" className="me-2 ms-2" />
        {icon2} Bayar <img src={rectangle} alt="rectangle" className="me-2 ms-2" />
        {icon3} Tiket
      </>
    );
  };
        ...
        <Row className="justify-content-between pay-track">
          <Col className="text-start">
            <Link
              ...
              <BsArrowLeftShort size={25} /> {bankName ? bankName : 'Pembayaran'}
            </Link>
