  ...
  const handleDelete = (payload) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire('dibatalkan!', 'Pesanan Anda Berhasil dibatalkan.', 'success').then(() => {
          dispatch(customerGetAllOrder(params));
        });
      }
    });
  ...
  return (
    <>
      <div className="container container-order-status">
        <div>
          <Tabs defaultActiveKey="payment-all" id="justify-tab-example" className="mb-3" justify>
            <Tab eventKey="payment-all" title="Semua">
              <AllPaymentStatus data={data} cars={cars} handleDelete={handleDelete} />
            </Tab>
            <Tab eventKey="payment-pending" title="Belum Bayar">
              <NotPaidPaymentStatus data={data} cars={cars} handleDelete={handleDelete} />
            </Tab>
